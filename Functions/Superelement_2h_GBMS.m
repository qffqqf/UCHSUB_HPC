    ...
    methods
        ...
        function [obj, children] = find_map(obj, ele_pack, obj_temp)
            if isempty(obj_temp)
                ...
                dofs_R = dofs_BR(end) + [1:numel(ele_br.dofs_red.R)];
                dofs_TR = dofs_R(end) + [1:numel(ele_br.dofs_red.TR)];
                dofs_T1 = dofs_TR(end) + [1:numel(ele_bl.dofs_red.T)];
                dofs_T2 = dofs_T1(end) + [1:numel(ele_bl.dofs_red.TR)];
                dofs_T3 = dofs_T2(end) + [1:numel(ele_br.dofs_red.T)];
                dofs_TL = dofs_T3(end) + [1:numel(ele_bl.dofs_red.TL)];
                dofs_L = dofs_TL(end) + [1:numel(ele_bl.dofs_red.L)];
                nI1 = numel(ele_bl.dofs_red.R);
                dofs_I1 = dofs_L(end) + [1:nI1];
                dofs_I2 = dofs_I1(end) + [1:numel(ele_bl.dofs_red.I)];
                dofs_I3 = dofs_I2(end) + [1:numel(ele_br.dofs_red.I)];
                obj.dofs.BL = dofs_BL;
                ...
                obj.ev_map = cell(2,1);
                obj.ev_map{1} = [dofs_BL,dofs_B1,dofs_B2,dofs_I1,dofs_T2,dofs_T1,dofs_TL,dofs_L,dofs_I2];
                obj.ev_map{2} = [dofs_B2,dofs_B3,dofs_BR,dofs_R,dofs_TR,dofs_T3,dofs_T2,dofs_I1,dofs_I3];
                children = {ele_bl, ele_br};
            else
                ele_bl = ele_pack{1};
                ele_br = ele_pack{2};
